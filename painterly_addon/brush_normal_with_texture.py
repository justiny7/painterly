...
def sample_texture(uv, combined_texture_pixels_data, combined_texture_image):
    if combined_texture_pixels_data is not None:
        pixel_x = int(uv.x * combined_texture_image.size[0]) % combined_texture_image.size[0]
        pixel_y = int(uv.y * combined_texture_image.size[1]) % combined_texture_image.size[1]
        sampled_color = combined_texture_pixels_data[pixel_y, pixel_x]
        return sampled_color
    return [0.5, 0.5, 0.5, 1]  # Default grey color if no texture
...
def create_complete_coverage_painterly_maps(stroke_width_range=(8, 15), stroke_length_range=(20, 40)):
    ...
    for obj_index, obj in enumerate(selected_objects):
        ...
        # Get object color and texture
        obj_color = get_object_color(obj)
        combined_texture_image, combined_texture_pixels_data = get_object_texture(obj)
        ...
        for grid_y in range(0, resolution, grid_size):
            for grid_x in range(0, resolution, grid_size):
                ...
                # Sample texture color if available
                if combined_texture_pixels_data is not None:
                    uv_coord = pixel_to_uv(x, y, resolution)
                    sampled_color = sample_texture(uv_coord, combined_texture_pixels_data, combined_texture_image)
                    varied_color = np.array(sampled_color)
                else:
...
def get_object_texture(obj):
    if obj.active_material and obj.active_material.use_nodes:
        for node in obj.active_material.node_tree.nodes:
            if node.type == 'TEX_IMAGE' and node.image and 'color' in node.image.name.lower():
                image = node.image
                pixels_data = np.array(image.pixels[:]).reshape((image.size[1], image.size[0], 4))
                return image, pixels_data
    return None, None

def pixel_to_uv(x, y, resolution):
    return Vector((x / resolution, y / resolution))
