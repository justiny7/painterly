...
def create_painterly_maps_with_shared_texture(
    stroke_width_range=(8, 15),
    stroke_length_range=(20, 40),
    normal_angle_threshold=10.0,
    color_variation=0.0
):
    """
    Apply the painterly effect to the provided objects.
    
    Parameters:
    -----------
    objects : list
        List of Blender objects to process
    stroke_width_range : tuple
        Min and max stroke width (min, max)
    stroke_length_range : tuple
        Min and max stroke length (min, max)
    normal_angle_threshold : float
        Threshold angle for normal map effects
    color_variation : tuple
        Min and max color variation (min, max)
    """
    start_time = time()
