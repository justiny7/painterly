...
def create_complete_coverage_painterly_maps(stroke_width_range=(8, 15), stroke_length_range=(20, 40)):
    ...
    print(f"Parameters: width={stroke_width_range}, length={stroke_length_range}")
    ...
    normal_map = bpy.data.images.new("NormalMap_Painterly", width=1024, height=1024)
    normal_map.colorspace_settings.name = 'Non-Color'
    
    color_map = bpy.data.images.new("ColorMap_Painterly", width=1024, height=1024)
    
    # Initialize numpy arrays for images
    normal_array = np.zeros((1024, 1024, 4), dtype=np.float32)
    normal_array[:, :, 0] = 0.5  # R - X
    normal_array[:, :, 1] = 0.5  # G - Y
    normal_array[:, :, 2] = 1.0  # B - Z
    normal_array[:, :, 3] = 1.0  # A - Alpha
    
    color_array = np.ones((1024, 1024, 4), dtype=np.float32)
    
    # Create a mask to track which pixels belong to which object
    object_ownership = np.full((1024, 1024), -1, dtype=np.int32)
    ...
    for obj_index, obj in enumerate(selected_objects):
        ...
        for face in bm.faces:
            ...
            pixel_uvs = [(int(uv.x * (1024-1)), int(uv.y * (1024-1))) for uv in uvs]
            ...
            for i in range(1, len(face.loops) - 1):
                ...
                max_x = min(1024-1, max(x for x, y in tri_uvs))
                min_y = max(0, min(y for x, y in tri_uvs))
                max_y = min(1024-1, max(y for x, y in tri_uvs))
    ...
    for obj_index, obj in enumerate(selected_objects):
        ...
        coverage_grid = np.zeros((1024 // grid_size + 1, 1024 // grid_size + 1), dtype=bool)
        ...
        for grid_y in range(0, 1024, grid_size):
            for grid_x in range(0, 1024, grid_size):
                # Check if any pixel in this grid cell belongs to the object
                cell_has_obj_pixel = False
                
                for y in range(grid_y, min(grid_y + grid_size, 1024)):
                    for x in range(grid_x, min(grid_x + grid_size, 1024)):
                ...
                # Add randomness to direction (similar to angle_variation in test file)
                angle = math.atan2(tangent.y, tangent.x) + random.uniform(-math.radians(45/2), math.radians(45/2))
                ...
                # Add edge noise like in the test file
                edge_noise_amount = 2.0  # Use the parameter directly
                stroke_width += random.uniform(-edge_noise_amount, edge_noise_amount)
                stroke_length += random.uniform(-edge_noise_amount, edge_noise_amount)
                ...
                # Draw stroke with enhanced normal map values
                normal_color = [
                    normal.x * 0.5 + 0.5,  # Enhance normal X component
                    normal.y * 0.5 + 0.5,  # Enhance normal Y component
                    normal.z * 0.7 + 0.3,  # Enhance normal Z component for more depth
                    1.0
                ]
                ...
                draw_brush_stroke(normal_array, x0, y0, x1, y1, normal_color, stroke_width, 1024, object_ownership, obj_index)
                draw_brush_stroke(color_array, x0, y0, x1, y1, varied_color, stroke_width, 1024, object_ownership, obj_index)
        ...
        total_cells = np.size(coverage_grid)
        print(f"  Initial coverage: {covered_cells}/{total_cells} grid cells")
        
        # Add some random strokes for variety (similar to sparsity in test file)
        sparsity_factor = 0.001  # Use the parameter directly
        num_random_strokes = max(10, int(len(obj_pixels) * sparsity_factor))  # At least 10 random strokes
        ...
        for _ in range(num_random_strokes):
            ...
            draw_brush_stroke(normal_array, x0, y0, x1, y1, normal_color, stroke_width, 1024, object_ownership, obj_index, opacity=0.6)
            draw_brush_stroke(color_array, x0, y0, x1, y1, varied_color, stroke_width, 1024, object_ownership, obj_index, opacity=0.6)
...
def draw_brush_stroke(image_array, x0, y0, x1, y1, color, thickness, resolution, mask=None, obj_index=None, opacity=0.8):
    ...
    while True:
        ...
        if mask is not None and np.any(ownership_mask):
